Properties {
	...
	$solution_name = "$base_dir\WebApiContrib.Formatting.Html.sln"
	$test_dll = "$build_artifacts_dir\WebApiContrib.Formatting.Html.Tests.dll"
...
Task NuGetBuild {
	& $nuget_exe pack "$base_dir/src/WebApiContrib.Formatting.Html/WebApiContrib.Formatting.Html.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
